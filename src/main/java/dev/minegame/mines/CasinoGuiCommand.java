...
public final class CasinoGuiCommand implements CommandExecutor {
    ...
    private Object stationValue(String g,String k,String path){if(g.equals("fights"))for(FightStationData s:fights.stations())if(s.key().equals(k)&&path.equals("fights.fighter-count"))return s.fighterCount();if(g.equals("slots"))for(SlotStationData s:slots.stations())if(s.key().equals(k))return switch(path){case "slots.reel-count"->s.reelCount();case "slots.row-count"->s.rowCount();default->globalValue(g,path);};return globalValue(g,path);}
    private Object globalValue(String g,String path){if(g.equals("minegame"))return mines.getCurrentConfigValue(path);if(g.equals("roulette"))return roulette.getCurrentConfigValue(path);if(g.equals("fights"))return fights.getCurrentConfigValue(path);return slots.getCurrentConfigValue(path);}
